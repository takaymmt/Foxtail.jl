...
"""
    KAWA(ts::TSFrame, period::Int=10; field::Symbol=:Close, fast::Int=2, slow::Int=30)
...
# Returns
- `TSFrame`: New TSFrame containing KAMA values with column name "KAWA2_period"
...
kama_custom = KAWA(ts, 15, field=:Close, fast=3, slow=40)
```
...
See also: [`_KAWA`](@ref) for the underlying implementation details.
"""
# Define and export public interface function
function KAWA(ts::TSFrame, period::Int=10; field::Symbol=:Close, fast::Int=2, slow::Int=30)
    prices = ts[:, field]
    results = _KAWA2(prices, period, fast, slow)
    col_name = Symbol(:KAWA2, :_, period)
    return TSFrame(results, index(ts), colnames=[col_name])
end
export KAWA

function _KAWA(data::AbstractVector{T}, n::Int=10, fast_ef::Int=2, slow_ef::Int=30) where T <: AbstractFloat
    ...
    fast_sc = T(2) / (fast_ef + T(1))
    slow_sc = T(2) / (slow_ef + T(1))
